...
2. 回答性: 見出し構造（H1/H2）、要点サマリ、定義文、箇条書きの充実度
3. FAQ/HowTo: FAQやHowToコンテンツの有無と充実度
4. E-E-A-T: 著者/運営者情報、問い合わせ、会社情報、更新日、参照リンク
5. 構造化データ: Schema.org（FAQPage/HowTo/Product/Article/Breadcrumb等）の有無
6. コンテンツ一貫性: 同一テーマでの網羅性、コンテンツの厚み
...
def analyze_page_with_llm(url: str, page_text: str, scores: dict) -> str:
    ...
    新仕様: 問題点（最大3つ）、改善TODO（最大3つ）、優先度、理由を出力。
    """
    if not (subscription_key and endpoint and deployment):
...
- FAQ/HowTo: {scores.get("FAQ/HowTo", 0)}
- E-E-A-T: {scores.get("E-E-A-T", 0)}
...
def analyze_domain_with_llm(results: list) -> str:
    """
    ドメイン全体の示唆を生成する。
    上位3課題、改善ロードマップ（2週間/1ヶ月/3ヶ月）、最初に直すべきURL Top10を出力。
    """
    if not (subscription_key and endpoint and deployment):
        return "Azure OpenAI API情報が設定されていません。"
    ...
    for result in results:
        ...
        summary_lines.append(
            ...
            f"E-E-A-T:{result.get('E-E-A-T',0)}, "
